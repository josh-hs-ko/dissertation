...
To actually harvest externalist composability, we need conversion isomorphisms in which the externalist predicates involved are pointwise conjunctions (as in the case of externalist ordered vectors in \autoref{sec:externalism-and-internalism}).
...
This example helps to emphasise that the notion of refinements is \key{proof-relevant}: an underlying value can have more than one promotion proofs, and consequently the more informative type in a refinement can have more inhabitants than the basic type does.
...
\end{itemize}
The refinement mechanism's purpose of being is thus to express intensional (representational) optimisations of the canonical promotion predicate, such that it is possible to work on just the residual information of the more refined type that is not present in the basic type.
...
\end{code}
We say that |a : A| and |b : B| are \key{in coherence} when |forget b ≡ a|, i.e., when |a|~underlies~|b|.
...
\item a coherence proof |c : (a : A) (p : P a) → C a (u a p)| saying that the result of promoting |a : A| must be in coherence with~|a|.
\end{itemize}
As an \Agda\ record datatype:
\begin{code}
...
The upgrading operation takes a function |f : A → B| and combines it with a promotion proof to get a function |f' : A' → B'|, which should transform underlying values in a way that is in coherence with~|f|.
That is, as |f'|~takes |a' : A'| to |f' a' : B'| at the more informative level, correspondingly at the underlying level, the value underlying~|a'|, i.e., |Refinement.forget r a' : A|, should be taken by~|f| to a value in coherence with~|f' a'|.
We thus define the statement ``|f'|~is in coherence with~|f|'' as
\begin{code}
...
A further example on upgrades (about insertion into a binomial heap) is given in \autoref{sec:binomial-heaps}.
...
We also define an alternative name |InvImage = _⁻¹_| to make partial application look better.
...
For example, we should be able to compose the two refinements from lists to ordered lists and vectors to get a refinement from lists to ordered vectors whose promotion predicate is the pointwise conjunction of the promotion predicates of the two refinements.
...
(We sometimes refer to |μ E| (e.g., lists) as an \key{ornamentation} of |μ D| (e.g., natural numbers).)
...
\end{code}
For each type |ListS A as|, there is exactly one (canonical) inhabitant (hence the name ``singleton datatype''~\citep{Monnier-singleton-types}), which is devoid of any horizontal content and whose vertical structure is exactly that of~|as|.
...
\end{code}
which is called the \key{refinement semantics} of ornaments~(\autoref{sec:optimised-predicates}).
...
We are all familiar with the idea of \key{positional number systems}, in which we represent numbers as a list of digits.
...
\end{code}
\caption{Merging two leftist heaps. Proof terms about ordering are coloured grey to aid comprehension (taking inspiration from --- but not really employing --- \varcitet{Bernardy-color}{'s ``type theory in color''}).}
\label{fig:merge}
\end{sidewaysfigure}
...
This thesis defines ornaments as relations between descriptions (indexed with an erasure function), and rebrands \citeauthor{Dagand-functional-ornaments}'s ornaments as ornamental descriptions.
...
It is thus actually biased to refer to the less informative end of an ornament as ``basic'', but the examples in this thesis are indeed biased in this sense, being influenced by \citeauthor{McBride-ornaments}'s original formulation.

\citet{Dagand-functional-ornaments} later adapted the original ornaments to index-first datatypes, and also proposed ``reornaments'' as a more efficient representation of promotion predicates, taking full advantage of index-first datatypes.
Reornaments are reimplemented in this thesis as optimised predicates using parallel composition, as a result of which we can derive properties about optimised predicates using pullback properties of parallel composition in \autoref{chap:categorical}.
...
The upgrade mechanism can thus be more easily extended by defining new combinators (which we actually do in \autoref{chap:algebraic}).
...
\todo[inline]{comparison with \citet{Bernardy-color} (erasure vs full computation power of ornaments); parametricity~\citep{Wadler-theorems-for-free, Bernardy-parametricity-and-dependent-types}}

%The idea of viewing vectors as promotion predicates was first proposed by \citet[page~82]{Bernardy-thesis}, which refers to the realisability transformation defined for pure type systems by \citet{Bernardy-realizability-parametricity}.
%The idea is later generalised to ``type theory in color''~\citep{Bernardy-color}, which uses modalities inspired by colors in typing to manage relative irrelevance of terms and erasure of irrelevant terms.
%For simple applications like the ones offered in \autoref{sec:ornament-refinement-examples}, type theory in color and ornamentation offer similar approaches, except that the former provides more native support for erasure and parametricity.
%Ornaments, however, are fully computational due to the presence of deletion~(|∇|), and can specify relationship between datatypes beyond erasure.

It is worth noting that the ornament--refinement framework is merely one small step towards an internalist library.
Parallel composition provides logical support for manufacturing composite internalist datatypes, but eventually the central problem is about how properties of actual data structures can be dissected and presented modularly in a useful way.
For example, we decomposed leftist heap-ordering into the leftist property and heap ordering for good reasons: there are operations meaningful for heap-ordered trees without the leftist property, and we can impose different leftist properties on these heap-ordered trees while reusing the operations previously defined for heap-ordered trees.
In contrast, it is perfectly logical to analyse red-black trees as internally labelled trees satisfying the red and black properties, but the red or black property by itself is useless in practice, and hence it is pointless to develop modules separately for the red and black properties.
Dependently typed programming is a new paradigm that grants data structural properties and their proofs the same status as programs, 