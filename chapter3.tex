...
\todo[inline]{residual view, partitioning view (fine vs coarse)}

A \emph{refinement} from a type~|X| to a type~|Y| is a \emph{promotion predicate} |P : X → Set| and a \emph{conversion isomorphism} |i : Y ≅ Σ X P|.\todo{universe polymorphism}
\begin{code}
...
\todo[inline]{a type refines another in the sense of being more informative rather than merely being a subset}
...
We will not exploit the complete refinement structure on indices, though, so in the actual definition of \emph{refinement families} below, the index-level refinement degenerates into the forgetful computation.
\begin{code}
...
\end{code}
A proof of |Ordered A _≤A_ b as| consists of exactly the inequality proofs necessary for ensuring that |as|~is ordered and bounded below by~|b| --- the representation of such a proof is optimised, justifying the name ``optimised predicate''.}
...
\todo[inline]{summary of the three-level architecture of ornaments, refinements, and upgrades; bundle; why ornaments?}