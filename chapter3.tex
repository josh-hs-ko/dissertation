...
Refinements are less useful when we move on to function types, however.
A presupposition here is that we want refinement relationship between function types to be compositional --- for example, we are interested in stating how the function type |List Nat → List Nat| refines the function type |Nat → Nat| in terms of the refinement between their component types |Nat| and |List Nat| (which states that the underlying natural number of a list is its length): a function |f : Nat → Nat| underlies another function |g : List Nat → List Nat| exactly when |g|~transforms the underlying natural number in the same way as~|f|, i.e., |length (g xs) ≡ f (length xs)| for all |xs : List Nat|.
...
Ṗ-toList : {X : I → List} → (X ⇉ const J) → (is : List I) → Ṗ is X → List J
Ṗ-toList f []        tt        = []
...
Ṗ-toEq : (is : List I) (js : Ṗ is (Inv e)) → Ė e (Ṗ-toList und is js) is
Ṗ-toEq []        tt        = []
