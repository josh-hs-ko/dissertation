...
To gain more flexibility, the specification language was later generalised to \key{relational programs}.
...
To emphasise the computational interpretation of relations, we will mainly model a relation between sets |A|~and~|B| as a function sending each element of~|A| to a \key{subset} of~|B|.
...
While |℘|~cannot be made into a conventional monad~\citep{Moggi-monads, Wadler-essence} because it is not an endofunctor, it can still be equipped with the usual monadic programming combinators, giving rise to a \key{relative monad}~\citep{Altenkirch-relative-monads}:
\begin{itemize}
...
\todo[inline]{Revise using upgrades.}

As a first example of bridging internalist programming with relational calculation through algebraic ornamentation, let us consider the \key{Fold Fusion Theorem}~\cite[Section~6.2]{Bird-AoP}:
...
In the rest of this section, we prove the \key{Streaming Theorem}~\citep[Theorem~30]{Bird-arithmetic-coding} by implementing the streaming algorithm given by the theorem with algebraic ornamented lists such that the algorithm satisfies its metamorphic specification by construction.
...
\begin{itemize}
\item apply~|g| to~|x| to \key{produce}~|b| and reach a new state~|x'|, and then apply~|h| to \key{consume} the list and update the state to~|h x'|, or
\item apply~|h| to \key{consume} the list and update the state to~|h x|, and then apply~|g| to~|h x| to \key{produce} an element and reach a new state,
\end{itemize}
...
The key to refining |min-coin-change| to the converse of a fold lies in the following version of the \key{Greedy Theorem}, which is essentially a specialisation of \varcitet{Bird-AoP}{'s Theorem~10.1}:
...
We define two \key{views}~\citep{McBride-view} --- or ``customised pattern matching'' --- to aid the analysis:
\begin{itemize}
