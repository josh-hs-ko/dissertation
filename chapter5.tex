...
upg-⊆ : Upgrade  ({i : I} → μ D i → μ D i)
                 (  {i : I} {x : X i} → μ ⌊ algOD D S ⌋ (i , x) →
                    {y : Y i} → R x y  → μ ⌊ algOD D T ⌋ (i , y))
upg-⊆ =  (fixedforall'(i ∶ I)) (newforall'(x ∶ X i)) rS i x ⇀ 
         (newforall'(y ∶ Y i)) (newforall(_ ∶ R x y)) toUpgrade (rT i y)
  where
    rS : (i : I) (x : X i) → Refinement (μ D i) (μ ⌊ algOD D S ⌋ (i , x))
    rS i x = toRefinement (algOD-FSwap D S (ok (i , x)))
    rT : (i : I) (y : Y i) → Refinement (μ D i) (μ ⌊ algOD D T ⌋ (i , y))
    rT i y = toRefinement (algOD-FSwap D T (ok (i , y)))
\end{code}
...
upg-⊇ =  (newforall'(i ∶ I)) (newforall'(y ∶ Y i)) rT i y ⇀
         (newSigma(x ∶ X i)) toUpgrade (rS i x) ×⁺ R x y
  where
    rS : (i : I) (x : X i) → Refinement (μ D i) (μ ⌊ algOrn D S ⌋ (i , x))
    rS i x = toRefinement (algOD-FSwap D S (ok (i , x)))
    rT : (i : I) (y : Y i) → Refinement (μ D i) (μ ⌊ algOrn D T ⌋ (i , y))
    rT i y = toRefinement (algOD-FSwap D T (ok (i , y)))
\end{code}
