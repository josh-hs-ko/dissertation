...
(For example, the type of |Fun-graph| is implicitly instantiated as |Graph {1} {0}|, since both |Set| is of type |Set₁| and any |A → B| (where |A|, |B : Set|) are of type |Set| ($=$~|Set₀|), and |Graph {1} {0}| itself is of type |Set₂|, whose level is computed by taking the successor of the maximum of the two level arguments.)
...
Fortunately, ornaments exhibit useful categorical structure, and we are able to tame the complexity of ornaments by reasoning abstractly in terms of this categorical structure.
