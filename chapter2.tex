...
\end{center}
From the logic perspective, predicates on |Nat| are a special case of |Nat|-indexed families of sets like~|P|; |Nat-elim| then delivers the induction principle for natural numbers, as it produces a proof of~|P n| for any |n : Nat| if proofs |z|~and~|s| of the base case and the inductive case can be constructed.
...
trans x y z p q = ≤N-elim y  ((lambda(y' _)) x ≤N y → x ≤N y')
                             ((lambda(p')) p') ((lambda(w r ih p')) step w (ih p')) z q p
\end{code}
...
data ^^^List (A : Set) : Set where
  ...
  ^^^_∷_  : A → List A → List A
\end{code}
and (throughout this dissertation) let |^^^Val : Set| be equipped with a decidable total preorder, i.e., there is a relation
\begin{code}
^^^_≤_ : Val → Val → Set
\end{code}
...
UIP refl refl = refl
\end{code}
In particular, in definitions of specific categories in \autoref{chap:categorical}, we employ McBride's ``John Major'' heterogeneous equality~\citep{McBride-thesis}, which was shown by McBride to be equivalent to propositional equality equipped with uniqueness of identity proofs.
...
\begin{code}
indexfirst data ^^^Vec' (A : Set) : Nat → Set where
  Vec' A n  offers  ^^^nil (neq : zero ≡ n)
            or      ^^^cons  (a : A) (m : Nat) (meq : suc m ≡ n) (as : Vec' A m) 
\end{code}
...
\block{Remark}{recursive families}{Vectors can also be defined as a ``recursive family''~\citep{Capretta-recursive-families}, i.e., an indexed family of types defined by induction on the index:
\begin{code}
^^^Vec'' : Set → Nat → Set
Vec'' A zero     = ⊤
Vec'' A (suc n)  = A × Vec'' A n
\end{code}
Recursive families are not subject to the usual ``strict positivity'' condition imposed on inductive datatypes (see, e.g., \citet{Coquand-CiC}).
With recursive families, however, the index in the type of a sub-object has to be structurally smaller, whereas inductive families do not have this restriction.
An example is the |BHeap| datatype in \autoref{sec:binomial-heaps}.}
...
%\block{Remark}{propositional equality}{Note that we cannot define propositional equality by an index-first datatype, but must treat propositional equality as a separate (perhaps primitive) notion.
%This, however, is in line with the approaches to propositional equality taken by observational type theory~\citep{Altenkirch-OTT} and homotopy type theory~\citep{UFP-HoTT} (see \autoref{sec:equality}), and allows us to choose from the different approaches (including the \Agda\ approach, which defines propositional equality by an inductive family).}
...
(|^^^⊤|~is a singleton set whose only inhabitant is~``|^^^tt ^^^⊤tt|'', and |_×_|~is cartesian product.)
If we flip the order of the arguments of |VecF A|:
\begin{code}
^^^VecF' A : Nat → (Nat → Set) → Set
VecF' A zero     =  (lambda(X)) ⊤
VecF' A (suc n)  =  (lambda(X)) A × X n
\end{code}
...
\block{Remark}{presentation of universes and their decoding}{We always present universes (e.g., |RDesc|) along with their decoding (e.g., |⟦_⟧| for |RDesc|) to emphasise the meaning of the codes, even when the decoding is not logically tied to the codes (cf.~\varcitet{ML-TT84}{'s universe}, which is inductive-recursive~\citep{Dybjer-induction-recursion} and must be presented with its decoding simultaneously).}
...
record  case  proj₁  = a
        sep   proj₂  = x endcase
\end{code}
(where |a : A| and |x : X a|) --- a cartesian product |A ^^^× ^^^_×_ ^^^_×_ B| is thus a special case, which is defined as |Σ A ((lambda(_)) B)|.
...
(x ∷ xs)  ++ ys = x ∷ (xs ++ ys)
\end{code}
\sidenote{\Agda\ supports such definitions of higher-level terms by ``pattern synonyms'', which we do not explicitly use in this dissertation.}
...
\end{code}
(The function |^^^const| is defined as |(lambda(x y)) x|.)}
...
In type theory, this can be modelled by |Σ|-types.
...
\end{code}
\sidenote{Note that the variable of type |b' ≤ y| (with spaces around~`|≤|') is named |b'≤y| (without spaces) --- a subtle but useful convention.}
...
This analysis of internalist datatypes and its application to modular library structuring is explored in \autoref{chap:refinements-and-ornaments} (in particular, the insertion example is concluded in \autoref{sec:insertion}).

The interconnection between internalism and externalism (in the form of conversion isomorphisms) also sheds some light on supporting internalist type design.
