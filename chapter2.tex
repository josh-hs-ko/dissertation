...
\end{code}
\sidenote{The implicit argument~|A| is explicitly displayed in curly braces on the left-hand side of the equation since we need to refer to~|I| on the right-hand side.}
...
\end{code}
An inhabitant of |Σ A X| is a pair where the type of the second component depends on the first component, and is written by listing values for the fields like
\begin{code}
record  case  proj₁  = a
        sep   proj₂  = x endcase
\end{code}
...
(x ∷ xs)  ++ ys = x ∷ (xs ++ ys)
\end{code}
\sidenote{\Agda\ supports such definitions of higher-level terms by ``pattern synonyms'', which we do not explicit use in this thesis.}
