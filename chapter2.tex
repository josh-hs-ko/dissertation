...
\item \Agda-specific syntax, notational conventions, and basic constructions used throughout the thesis are also introduced.
...
This is called the \key{propositions-as-types principle}, which reflects the observation that proofs are nothing but a certain kind of mental constructions.
...
\end{center}
\sidenote{The type of~|s| illustrates \Agda's syntax for dependent functions --- the value~|n| of the first argument is referred to in the types of the second argument and the result.}
...
In \key{dependently typed} languages based on type theory, however, the two traditional levels are coherently integrated into one, so programs can be naturally constructed along with their correctness proofs.
...
The propositions-as-types principle, however, can lead to a more intimate form of program correctness by construction by blurring the distinction between programs and proofs even further; this form of program correctness --- called \key{internalism} --- is introduced in \autoref{sec:externalism-and-internalism}, which opens the central topic studied by this thesis.
...
\end{code}
\sidenote{An \Agda\ name with underscores (like~|_≤N_|) can be applied to arguments either normally (like ``|_≤N_ m n|'') or by substituting the arguments for the underscores with proper spacing (like ``|m ≤N n|'').}
...
data List (A : Set) : Set where
  ...
  _∷_  : A → List A → List A
\end{code}
and (throughout this thesis) let |Val : Set| be equipped with a decidable total ordering, i.e., there is a relation
\begin{code}
_≤_ : Val → Val → Set
\end{code}
...
\end{code}
\sidenote{Quantifications like |{x : Val}| are implicit arguments to a function, which can be omitted when applying the function, and \Agda\ would try to infer them.}
...
The witnesses carried by |yes| and |no| are ignored in this case \sidenote{whose names are suppressed by underscores}, but in general these can be proofs that are further matched and change the context and the goal type.
(Admittedly, this is a trivial example with regard to the full power of |with|-matching, but |insert| will be used in later examples in this thesis.)
}
...
Specifically, \name{Epigram} syntax makes it clear which and in what order eliminators are invoked, so programs are easily guaranteed to be based on elimination --- and thus are terminating --- while remaining readable.
...
The resulting type theory is called an \key{intensional type theory}, whose characteristic feature is decidable typechecking (enabling effective recognition of set membership) due to decidability of judgemental equality.
...
While we can explicitly work with pointwise equality on functions, not being able to identify extensionally equal functions propositionally means that we do not automatically get basic properties like
\begin{code}
...
\item A new direction is being pursued by \key{homotopy type theory}~\citep{UFP-HoTT}, which gives propositional equality a higher-dimensional homotopic interpretation and broaden its scope with the univalence axiom and higher inductive types, but the computational meaning of the theory remains an open problem.
...
For this thesis, however, we confine ourselves to a basic setting in which we freely invoke \key{uniqueness of identity proofs}, which is definable in \Agda\ by pattern matching:
\begin{code}
...
With only \Agda's intensional equality, we have to explicitly work with equality-like propositions (like pointwise equality on functions) and manage them with the help of \key{setoids}~\citep{Barthe-setoids} in this thesis --- Chapters \ref{chap:categorical}~and~\ref{chap:equivalence}, specifically.
...
In this thesis we not only use but also need to \emph{compute} a class of inductive families which we call \key{index-first datatypes}~\citep{Chapman-levitation, Dagand-functional-ornaments}, and hence need to construct a universe for them.
...
\end{code}
\sidenote{The implicit argument~|I| is explicitly displayed in curly braces on the left-hand side of the equation since we need to refer to~|I| on the right-hand side.}
...
syntax Σ A (λ a → T) = (Σ'(a ∶ A)) T
\end{code}
An inhabitant |p : Σ A X| is a pair where the type of the second component depends on the first component --- a cartesian product |A × B| is thus a special case, which is defined as |Σ A ((lambda(_)) B)|.
...
\block{Remark}{first-order vs higher-order representation}{The functorial structures generated by descriptions are strongly reminiscent of \key{indexed containers}~\citep{Altenkirch-indexed-containers}; this will be explored and exploited in \autoref{chap:equivalence}.
...
\citet{Ko-OAOAOO} proposed to call this programming style \key{internalism}, suggesting that proofs are internalised in programs, while the traditional proving-after-programming style is called \key{externalism}.
...
\block*{Historical remark}{
The distinction between internalism and externalism at least goes back to simply typed $\lambda$-calculus á la Church and Curry --- a Church-style $\lambda$-term always appears with its type and they together denote a value of the type, whereas Curry-style $\lambda$-terms are untyped and all reside in a common domain, and a term is later stated to have additional properties via typing.
\citet{Reynolds-intrinsic-extrinsic-semantics} called the two styles as giving ``intrinsic'' and ``extrinsic'' semantics to a language, and established relationship between the two kinds of semantics via a logical relations theorem.
Our emphasis is on a more practical aspect about how intrinsic and extrinsic semantics can affect the attitude and approach to program construction.}
...
insert-EO y (xs , ord) b'≤y b'≤b = insert y xs , insert-ordered y ord b'≤y b'≤b
\end{code}
where |insert-ordered| proves that |insert| preserves ordering:
\begin{code}
insert-ordered :  (y : Val) {xs : List Val} {b : Val} → Ordered b xs →
...
For example, internalist and externalist vectors are related by the representation-converting isomorphisms:
\begin{code}
...
(The definition of~|_≅_|\,, which is actually an instance of a record type |Iso|, appears in \autoref{sec:categories-and-functors}.)
...
\path[->] (Vec-Val-n)        edge node[above]{|insert-V y|}                                (Vec-Val-suc-n)
          ...
          (ExtVec-Val-n)     edge node[above]{|insert y *|} node[below]{|insert-length y|} (ExtVec-Val-suc-n)
          (ExtVec-Val-suc-n) edge node[label on arrow]{|Iso.from (Vec-iso Val (suc n))|}   (Vec-Val-suc-n);
\end{tikzpicture} \]
Similarly, we can get insertion for internalist ordered lists and ordered vectors from the externalist library by suitable conversion isomorphisms of the same form as |Vec-iso|.
...
The interconnecting isomorphisms between internalism and externalism also shed some light on supporting internalist type design.
